...
class KontoData {
    ...
    async hentAlleKontiForBruger(brugerID) { //henter alle konti for en given bruger (brugerID)
        ...
        `); //SQL-forespørgsel, der henter alle konti, der er knyttet til den givne bruger

        return result.recordset; //returnerer en liste over alle konti for brugeren
    }
    ...
    async hentKontoMedID(kontoID) {
        ...
        return result.recordset[0]; //returnerer den første konto i resultatsættet (der vil kun være én konto)
    }
